...
twt_df = pd.read_csv(twt_df_path, sep="\t").fillna("")
# print(56709/65792*100, 1370/65792*100, 3131/65792*100, 4582/65792*100)
# print(30894/35154*100, 836/35154*100, 2083/35154*100, 1341/35154*100)

fake_satire = [tuple(row) for row in twt_df[["label", "text"]].values]
...
for train_ix, valid_ix in skf_splits:
    ...
    claim_fields = [('label', label_field), ('text', text_field)]
    ...
    for i in range(n_epochs):
        ...
        for step, train_batch in enumerate(train_iterator):
            ...
            text_ = train_batch.text
            text_ = [sent_tokenize(t)[:max_sents_] for t in text_]
            ...
            if (step + 1) % round(len(train_iterator) / checkpoints_per_epoch) == 0 or (step + 1) == len(train_iterator):
                ...
                train_f1_micro = f1_score(train_tgt_scoring, train_pred_scoring,
                                          labels=list(range(n_classes)), average='micro')
                train_f1_macro = f1_score(train_tgt_scoring, train_pred_scoring,
                                          labels=list(range(n_classes)), average='macro')
                print("Training F1 (weighted): " + str(train_f1.round(4)) + ", " +
                      "F1 (micro): " + str(train_f1_micro.round(4)) + ", " +
                      "F1 (macro): " + str(train_f1_macro.round(4)) + ", " +
                ...
                with torch.no_grad():
                    ...
                    valid_f1 = f1_score(valid_tgt_scoring, valid_pred_scoring,
                                        labels=list(range(n_classes)), average='weighted')
                    valid_f1_micro = f1_score(valid_tgt_scoring, valid_pred_scoring,
                                              labels=list(range(n_classes)), average='micro')
                    valid_f1_macro = f1_score(valid_tgt_scoring, valid_pred_scoring,
                                              labels=list(range(n_classes)), average='macro')
                    valid_loss = valid_epoch_loss / len(valid_iterator)
                    print("Validation F1 (weighted): " + str(valid_f1.round(4)) + ", " +
                          "F1 (micro): " + str(valid_f1_micro.round(4)) + ", " +
                          "F1 (macro): " + str(valid_f1_macro.round(4)) + ", " +
                          "epoch loss: " + str(round(valid_loss, 4)))
                    if valid_f1 > valid_f1_hiscore:
                        valid_f1_hiscore = valid_f1
    valid_metrics.append([valid_f1, valid_f1_micro, valid_f1_macro, valid_loss])
