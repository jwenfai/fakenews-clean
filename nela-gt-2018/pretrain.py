...
def pick_article(day_dir):
    # Pick random day, then random outlet (source), and finally random article
    source_dirs = os.listdir(articles_dir / day_dir)
    source_dir = np.random.choice(source_dirs)
    art_paths = os.listdir(articles_dir / day_dir / source_dir)
    art_path = np.random.choice(art_paths)
    return articles_dir / day_dir / source_dir / art_path


def articles_reader(art_paths):
    # BERT tokenizer automatically strips out newlines
    articles = []
    for art_path in art_paths:
        encoding = chardet.detect(open(art_path, "rb").read())["encoding"]
        with open(art_path, "r", encoding=encoding) as file:
            art_txt = file.read()  # reads as a single line, retains all newlines
        articles.append(art_txt)
    return articles
...
nelagt_dir = Path("K:/Work/Datasets-FakeNews/source-reliability/nela-gt-2018")
articles_dir = nelagt_dir / "articles"

model_params_dir = Path('K:/Work/ModelParams').expanduser()
model_trfmr_weights = str(model_params_dir / 'maskedlm_nelagt')
...
for i in range(n_epochs):
    ...
    for step in np.arange(steps_per_epoch):
        mlm.train()
        optimizer.zero_grad()
        rand_dates = np.random.choice(dates, batch_size_)
        art_paths_ = [pick_article(day) for day in rand_dates]
        batch_doc = articles_reader(art_paths_)
        if sum([len(doc) for doc in batch_doc]) > 0:
            batch_doc = [doc for doc in batch_doc if len(doc) > 0]
