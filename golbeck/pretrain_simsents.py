...
model_trfmr_weights = str(model_params_dir / 'maskedlm_golbeck_simsents')
...
texts = np.hstack([[" ".join([t] + similar_sents(t, b, False)) for t, b in fakes_df[["title", "body"]].values],
                   [" ".join([t] + similar_sents(t, b, False)) for t, b in satires_df[["title", "body"]].values]])
