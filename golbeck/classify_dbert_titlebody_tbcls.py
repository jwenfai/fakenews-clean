...
def prepare_text(text_title, text_body, output_type, max_body_sents):
    if output_type == "title-body":
        prepped_text = [[t, b] for t, b in zip(text_title, text_body)]
    elif output_type == "title-simsents":
        prepped_text = [[t] + [" ".join(similar_sents(t, b, max_body_sents))] for t, b in zip(text_title, text_body)]
    return prepped_text
...
class ModelTokenizer:
    ...
    def encode(self, art_list):
        ...
        # len(art_) in art_list is always 2 in the 2 CLS model
        title_tkn = [cls_token] + tokenizer.tokenize(art_list[0]) + [sep_token]
        body_tkn = [cls_token] + tokenizer.tokenize(art_list[1])[:(self.max_clm_len - len(title_tkn))] + [sep_token]
        art_tkn_list = title_tkn + body_tkn
...
class ClaimEvaluator(nn.Module):
    def __init__(self, num_labels, pretrained_weights="distilbert-base-uncased"):
        super().__init__()
        self.concat_dim = 768 * 2  # 1 for title, 1 for body
        self.pretrained_weights = pretrained_weights
        self.num_labels = num_labels
        self.classifier = SeqClassifier(self.num_labels, rep_dim=self.concat_dim, dropout=0.1)  # minus 1 claim
